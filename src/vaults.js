...
// FIX: mega_swap now emits { chain, swapUSD, log, poolAddr }
// bootstrap.js uses log + poolAddr to build arb parameters immediately
...
async function execute(chainName, svKey, calldata, profitEst) {
  ...
  if(!addr){ if(_sv[svKey])_sv[svKey].missed+=profitEst; emit('missed_rev',{chain:chainName,sv:svKey,amount:profitEst}); return null }
  ...
  try {
    ...
    try { const rev=await import('./revenue.js'); rev.depositToLPVault(profitEst) } catch{}
...
async function onMegaSwap(chainName, log, swapUSD) {
  ...
  // FIX: pass full log object + poolAddr so bootstrap/scanner can use it
  // This was the missing piece — scanner received undefined log previously
...
// Pool addresses vaults watches — matches what MEGA_POOLS is subscribed to
const MEGA_POOLS={
...
export function startVaults(){
  ...
  try { const saved=getConfig('sv_stats'); if(saved)Object.assign(_sv,JSON.parse(saved)) } catch{}
