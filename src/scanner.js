// Price gap scanner — real Uniswap V3 Swap events
// Decodes sqrtPriceX96 from SwapEvent.data
// Emits arb_opportunity when gap > threshold
...
// Verified pool addresses from on-chain data
const PAIRS = [
  ...
  { chain:'arbitrum', name:'ETH/USDC-500-3000',  asset:'weth', ft:'usdc',
  ...
  { chain:'base',     name:'ETH/USDC-500-3000',  asset:'weth', ft:'usdc',
  ...
  { chain:'polygon',  name:'ETH/USDC-500-3000',  asset:'weth', ft:'usdc',
...
const MIN = {
  ...
  base:{gap:0.01,profit:2}, polygon:{gap:0.01,profit:2},
  default:{gap:0.05,profit:10}
}
...
// Decode sqrtPriceX96 from Uniswap V3 Swap event
...
// sqrtPriceX96 is at bytes 128-192 of data (after amount0 and amount1)
function decodePrice(log, t0u) {
  try {
    const d = (log.data||'').replace('0x','')
    if (d.length < 320) return null
    // sqrtPriceX96 = uint160, stored at offset 64 in non-indexed data
    const sq = BigInt('0x'+d.slice(128,192))
    if (!sq) return null
    const f = Number(sq) / 2**96
    const p = t0u ? (1/(f*f))*1e12 : (f*f)*1e12
...
function evaluate(pair) {
  ...
  if (flash < 1e5) return
  const fees = (buy.fee/10000+sell.fee/10000)*100
  const slip = (flash/buy.tvl)*50+(flash/sell.tvl)*50
...
export function startScanner() {
  ...
  // CEX price bridge — uses real CEX price as synthetic for single-pool gaps
  ...
  // Mega-swap bridge — uses swap event from vaults
