...
export async function deployIdle(chainName) {
  try {
    ...
    const deploy = usdc - 500
    if (deploy < 50) return
    const amt = BigInt(Math.floor(deploy * 1e6))
    const appData = encodeFunctionData({
      abi: ERC20_ABI, functionName: 'approve', args: [chain.aavePool, amt]
    })
    await sendViaPimlico(chainName, chain.usdc, appData)
    const supData = encodeFunctionData({
      abi: SUPPLY_ABI, functionName: 'supply', args: [chain.usdc, amt, addr, 0]
    })
...
export function startYield() {
  setInterval(async () => {
    for (const c of ['polygon','arbitrum','avalanche']) {
