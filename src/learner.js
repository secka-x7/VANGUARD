...
export function getMinProfit(chain) {
  const { CHAINS } = await import('./config.js').catch(()=>({CHAINS:{}}))
...
export function getBestAssets(chain) {
  return query(
    `SELECT collateral_asset, AVG(profit_usdc) as avg, COUNT(*) as n
     FROM executions WHERE chain=? AND status='success'
     GROUP BY collateral_asset ORDER BY avg DESC LIMIT 5`, [chain]
  )
}
...
export function updateLearner() {
  ...
  for (const c of chains) for (const p of protocols) {
    const rows = query(
      `SELECT COUNT(*) as t, SUM(CASE WHEN status='success' THEN 1 ELSE 0 END) as w
       FROM executions WHERE chain=? AND protocol=?`, [c,p]
    )
    if (rows[0]?.t > 0) {
